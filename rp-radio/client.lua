...
Radio.Labels = {        
	{ "FRZL_RADIO_HELP",   "~n~~ ~ channel ~a~ numper ~n~channel: ~1~ MHz" },
	{ "FRZL_RADIO_HELP2", "" },
...
RegisterNUICallback('datasend', function(data, cd)  -- on/off
    local playerPed = PlayerPedId()
    local isFalling = IsPedFalling(playerPed)
    local isDead = IsEntityDead(playerPed)
...
RegisterNUICallback('datasend5', function(data, cd)
...
RegisterNUICallback('datasend3', function(data, cd)
...
RegisterNUICallback('datasend2', function(data, cd)
